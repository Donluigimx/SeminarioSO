...
class SistemaOperativo:
    ...
    def __init__(self, quantum):
        ...
        self.asignar_memoria(0, self.OS_SIZE)
        self.suspended_number = 0
        self.suspended_next_size = 0
        curses.start_color()
        curses.noecho()
        curses.cbreak()
    ...
    def procesosNuevoListo(self):
        ...
        while (int(ceil(self.procesoSiguiente['size'] / self.PAGE_SIZE))) <= self.page_number:
            self.asignar_memoria(self.procesoSiguiente['id'], self.procesoSiguiente['size'])
    ...
    def procesoTerminado(self, operacion = True):
        ...
        self.liberar_memoria(self.proceso['id'])
        if operacion:
    ...
    def asignar_memoria(self, id, size):
    ...
    def imprimir(self):
        ...
        for key, page in enumerate(self.pages):
            if page['id'] == -1:
                ...
                msg = "%d - %d / %d id = OS" % (key + 1, page['using'], page['size'])
            else:
            ...
            if [i for i in self.procesosBloqueados if i['proceso']['id'] == page['id']]:
                ...
                self.screen.addstr(2 + key, 90, msg, curses.color_pair(4))
            else:
                self.screen.addstr(2 + key, 90, msg)
        ...
        self.screen.addstr(28, 0, "Procesos Suspendidos: " + str(self.suspended_number))
        self.screen.addstr(29, 0, "Suspendido siguinte tamaño: " + str(self.suspended_next_size))
        self.screen.addstr(30, 0, "Bloques proceso siguiente: %d" % (int(ceil(self.suspended_next_size/self.PAGE_SIZE))))
        self.screen.addstr(32, 0, "Tiempo Global: " + str(self.tiempoGlobal))
    ...
    def suspend(self):
        if len(self.procesosBloqueados) > 0:
            try:
                io = open("suspended.json", mode='r')
                blocked = loads(io.read())
                proceso = self.procesosBloqueados.pop(0)
                self.liberar_memoria(proceso['proceso']['id'])
                blocked.append(proceso)
                self.suspended_next_size = blocked[0]['proceso']['size']
                self.suspended_number = len(blocked)
                dec_str = dumps(blocked)
                io.close()
                io = open("suspended.json", mode='w')
                io.write(dec_str)
                io.close()
    ...
    def undo_suspend(self):
        if os.path.isfile("suspended.json"):
            io = open("suspended.json", mode='r')
            dec = None
            val = io.read()
            if val is not None:
            ...
            if dec is None:
                ...
                proceso = dec.pop(0)
                if (int(ceil(proceso['proceso']['size'] / self.PAGE_SIZE))) <= self.page_number:
                    self.asignar_memoria(proceso['proceso']['id'], proceso['proceso']['size'])
                    proceso['tb'] = self.tiempoGlobal
                    self.procesosBloqueados.append(proceso)
                    self.suspended_number = len(dec)
                    if self.suspended_number == 0:
                        self.suspended_next_size = 0
                    else:
                        self.suspended_next_size = dec[0]['proceso']['size']
                    dec_str = dumps(dec)
                    io.close()
                    io = open("suspended.json", mode='w')
                    io.write(dec_str)
